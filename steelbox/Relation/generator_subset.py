...
def generator(config):
    ...
    count = 0
    import pickle
    with open('Relation/dataset.p','rb') as ff:
        images = pickle.load(ff)
        questions = pickle.load(ff)
        answers = pickle.load(ff)

    idex = pickle.load(open('data_sub/relation_tiers.p','rb'))
    idex = idex[:60000]
    print(idex)
    images, questions, answers = np.array(images)[idex], np.array(questions)[idex], np.array(answers)[idex]
...
def main():
    ...
    parser.add_argument('--dir_name', type=str, default='Sort-of-CLEVR_default_subset')
